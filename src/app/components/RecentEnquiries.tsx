    ...
    <div className="bg-white rounded-3xl p-6 shadow-sm ring-1 ring-gray-50">
      <div className="flex items-center justify-between mb-6">
        ...
        <Link
          ...
          className="text-xs font-bold text-[#a0004f] hover:text-black flex items-center gap-1 bg-pink-50/50 px-3 py-1.5 rounded-xl transition-all"
        >
      ...
      {loading ? (
        <div className="py-8 flex justify-center">
          <Loader2 className="w-6 h-6 animate-spin text-[#a0004f]" />
        </div>
      ) : enquiries.length === 0 ? (
            ...
            <tbody className="divide-y divide-gray-50">
              {enquiries.map((enquiry) => (
                <tr key={enquiry.id} className="hover:bg-gray-50/20 transition-colors">
                  ...
                  <td className="py-3.5">
                    <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-pink-50 text-pink-600 text-[10px] font-bold uppercase tracking-wider">
