...
export const TextAreaField: React.FC<TextAreaFieldProps> = ({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        toast.error("Invalid JSON format. Please upload a valid JSON file.");
      }
    };
  ...
  const textareaClass = `w-full px-6 py-4 bg-white border border-gray-200 rounded-2xl text-sm focus:ring-2 focus:outline-none focus:border-[#a0004f] focus:ring-1 focus:ring-[#a0004f] outline-none transition-all text-gray-800 ${className}`;
          ...
          <label className="text-[11px] font-bold text-gray-400 uppercase tracking-widest ml-4 flex items-center gap-1.5 relative">
            ...
            {tooltip && (
              <div className="group relative flex items-center">
                <HelpCircle className="w-3.5 h-3.5 cursor-help text-gray-300 hover:text-[#a0004f] transition-colors" />
                {/* Tooltip Bubble */}
          ...
          {allowJsonUpload && (
            <>
              <button
                ...
                className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-gray-700 hover:text-[#a0004f] transition-colors cursor-pointer"
          ...
          <form onSubmit={handleInsertLink} className="flex flex-col gap-3">
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                className="px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-xs focus:ring-1 focus:ring-[#a0004f] focus:border-[#a0004f] outline-none text-gray-800"
              />
            </div>
            ...
            <div className="flex gap-2 justify-end pt-2">
              ...
              <button
                type="submit"
                className="px-4 py-1.5 bg-[#a0004f] text-white rounded-lg text-xs font-semibold hover:bg-[#80003f] cursor-pointer transition-colors"
              >
                Insert
              </button>
            </div>
