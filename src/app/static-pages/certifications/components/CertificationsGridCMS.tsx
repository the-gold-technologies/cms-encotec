...
export function CertificationsGridCMS() {
  ...
  const handleCertChange = (index: number, field: "title" | "category" | "desc", value: string) => {
    setFormData((prev) => {
  ...
  const addCert = () => {
    setFormData((prev) => ({
      ...prev,
      certificationsList: [...prev.certificationsList, { title: "", category: "", desc: "" }]
    }));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (json.success) {
        toast.success("Accreditations Section saved successfully!", { id: toastId });
      } else {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 flex flex-col gap-4">
      <SectionHeader
        ...
        description="Manage the ISO, ASME, and IBR certificates displayed on the page."
        isOpen={isOpen}
      ...
      {isOpen && (
        <div className="flex flex-col gap-6 pt-4 border-t border-gray-50">
          <div className="flex justify-between items-center">
            <span className="text-sm font-semibold text-gray-700">Certificates list ({formData.certificationsList.length})</span>
            <button
              onClick={addCert}
            ...
            {formData.certificationsList.map((cert, idx) => (
              <div key={idx} className="p-4 border border-gray-100 rounded-xl flex flex-col gap-4 relative">
                ...
                <TextAreaField
                  ...
                  name={`desc-${idx}`}
                  value={cert.desc}
                  onChange={(e) => handleCertChange(idx, "desc", e.target.value)}
                  rows={2}
                  required
                />
              </div>
