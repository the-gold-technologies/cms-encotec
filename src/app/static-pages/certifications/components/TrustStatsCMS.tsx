...
export function TrustStatsCMS() {
  ...
  useEffect(() => {
    fetchWithCache("/api/certifications")
      .then((json) => {
        if (json.success && json.data?.TrustStats) {
          const data = json.data.TrustStats;
  ...
  const handleSave = async () => {
    setIsSaving(true);
    const toastId = toast.loading("Saving Compliance Stats Section...");
    try {
      const payload: any = {
        stats: statsList,
        statsList,
      ...
      if (json.success) {
        toast.success("Compliance Stats saved successfully!", { id: toastId });
      } else {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 flex flex-col gap-4">
      <SectionHeader
        title="Trust & Compliance Stats"
        description="Manage numeric thresholds and stat labels shown on the certifications page. Add or remove counter stats dynamically."
        isOpen={isOpen}
