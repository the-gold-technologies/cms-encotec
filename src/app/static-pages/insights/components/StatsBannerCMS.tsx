    ...
    fetchWithCache("/api/insights")
      .then((json) => {
        if (json.success && json.data?.StatsBanner) {
          setFormData({ ...defaultFormData, ...json.data.StatsBanner });
        }
      })
      .catch(console.error);
  }, []);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 flex flex-col gap-4">
      <SectionHeader
        ...
        description="Manage the 4 numeric values, suffixes, and descriptions shown on the stats grid."
        isOpen={isOpen}
            ...
            {Array.from({ length: 4 }).map((_, i) => {
              ...
              return (
                <div key={i} className="p-4 bg-gray-50/20 border border-gray-100 rounded-xl flex flex-col gap-4">
                  ...
                  <InputField
                    label="Suffix (e.g. +, K+)"
                    name={`stats${num}Suffix`}
                    value={(formData as any)[`stats${num}Suffix`]}
                    onChange={handleChange}
                  />
                  <InputField
                    label="Counter Label"
