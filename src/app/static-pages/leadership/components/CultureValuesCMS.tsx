...
export function CultureValuesCMS() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (json.success) {
        toast.success("Culture & Values Section saved successfully!", { id: toastId });
      } else {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 flex flex-col gap-4">
      <SectionHeader
        ...
        description="Manage the titles and description snippets of the 4 culture pillars."
        isOpen={isOpen}
        ...
        <div className="flex flex-col gap-6 pt-4 border-t border-gray-50">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {Array.from({ length: 4 }).map((_, i) => {
              const num = i + 1;
              return (
                <div key={i} className="p-5 bg-gray-50/20 border border-gray-100 rounded-xl flex flex-col gap-4">
                  <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider">Value pillar {num}</span>
                  <InputField
                    label="Pillar Title"
